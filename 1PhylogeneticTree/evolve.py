...
def run(prodfile='prod.npy',fin='datadn.txt',fout='./best',out2='top_k_trees',out3='clonal_frequencies',out4='llh_trace',num_samples=2500,mh_itr=5000,rand_seed=1,normfactor=1.00,purity=1.00,meandn=-2.0,sddn=0.2):
	...
	tssb  = TSSB( dp_alpha=dp_alpha, dp_gamma=dp_gamma, alpha_decay=alpha_decay, root_node=root, data=codes, num_up=num_gene, normfactor=normfactor,purity=purity,meandn=meandn,sddn=sddn)
	...
	rate_est	  = zeros((num_samples, num_gene))
	...
	for iter in range(-burnin,num_samples):
		...
		if iter>=0:
			...
			rate_est[iter]	  = exp(tssb.rate1)
	   
			
		if iter>=0:
			if True or mod(iter, 10) == 0:
				(weights, nodes) = tssb.get_mixture()
				print iter, len(nodes), cd_llh_traces[iter], tssb.dp_alpha, tssb.dp_gamma, tssb.alpha_decay#, " ".join(map(lambda x: "%0.2f" % x, intervals.tolist()))
				intervals = zeros((8))
...
if __name__ == "__main__":
	run(sys.argv[1],sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5],sys.argv[6],int(sys.argv[7]),int(sys.argv[8]),int(sys.argv[9]),float(sys.argv[10]),float(sys.argv[11]),float(sys.argv[12]),float(sys.argv[13]))
	#run('data.txt')
