...
def print_best_tree(fin,fout):
    ...
    #fout.write(tssb.root.get_ancestors())
	fout.write('\n\n')	
...
def print_node2(node, parent,tree,wts,fout):
	...
	if len(node['children'])==0 and node['node'].params<0.1: return
	...
	out_str = str(node_name) + ',\t' + str(around(node['node'].params,3)) + ',\t' + str(len(node['children'])) + ',\t' + str(len(genes)) + ',\t' + gnames + ',\t' + str(parent) + '\n'
	fout.write(out_str)
