...
class TSSB(object):
    ...
    def __init__(self, dp_alpha=1.0, dp_gamma=1.0, root_node=None, data=None, normfactor=None, purity=None, num_up=None, meandn=None, sddn=None, min_depth=0, max_depth=15, alpha_decay=1.0, mu_dn=-2.0, std_dn=0.3, num_dn=0):
        if root_node is None:
            raise Exception("Root node must be specified.")
        
        #data_up = data[data.a<data.b]
        #result = data['a']/data['d']    
        #print(data)
        self.num_up   = num_up
        self.num_dn   = num_dn
        ...
        self.rate1dn=self.rateprior(self._base_valuedn,self._stddn, self.num_dn) #self.num_data)   #need to change
        self.rate1 = numpy.concatenate((self.rate1up, self.rate1dn))
        self.rate=exp(self.rate1)
        root_node.tssb = self
        self.norm=[]
        self.norm.append(normfactor)
        self.purity=[]
        self.purity.append(purity)
        print(self.norm,self.purity)
        #self.norm.extend(normfactor2)
    ...
    def rateprior(self,base_value,std,dims):
        ...
        return m2
    #-0.5*log(2*std*math.pi)-(x-m)*(x-m)/(2*std) ###
    def normpdfln(self,x,m,std):
    ...
    def resample_mhypers(self, dp_rate1=True):
        ...
        def dp_m_llh(logratio,n):
            llh=0.0
            llh = llh + (self.assignments[n].logprob(self.data[n:n+1],exp(logratio),self.norm,self.purity)+self.normpdfln(self._base_valueup,logratio,self._stdup))
            return llh 
    ...
    def add_data(self, data):
        ...
        for n in range(num_new_data):
            ...
            for k, node in enumerate(nodes):
                ...
                logprobs.append( log(weights[k]) + node.logprob(data[n],self.rate[n]),self.norm,self.purity )
...
#        self.num_data    = 0
    ...
    def resample_assignments(self):
        ...
        for n in range(self.num_data):
            ...
            min_u = 0.0
            #print('n=',n)
            llh_s = log(rand()) + self.assignments[n].logprob(self.data[n:n+1],self.rate[n],self.norm,self.purity)

            while True:
                new_u                = (max_u-min_u)*rand() + min_u
                (new_node, new_path) = self.find_node(new_u)
                new_llh              = new_node.logprob(self.data[n:n+1],self.rate[n],self.norm,self.purity)
                if new_llh > llh_s:
    ...
    def complete_data_log_likelihood(self):
        ...
        llhs = 0.0
        for n in range(self.num_data):
            llhs = llhs + self.assignments[n].logprob(self.data[n:n+1],self.rate[n],self.norm,self.purity)
        #print('llhs',llhs,self.rate[n],self.assignments[n].logprob(self.data[n:n+1],self.rate[n],self.norm))
        return llhs
