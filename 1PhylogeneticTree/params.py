...
def metropolis(tssb,iters=1000,std=[0.01],burnin=0,ntps=1,fin=''):
	...
	NDELTA = str(len(tssb.data[0].a))
	NNODES = str(len(nodes))
	TREE_HEIGHT = str(max([node.ht for node in nodes])+1)
	NTPS = str(ntps)
	NRATE = list_to_string(tssb.rate)
	NORM= list_to_string(tssb.norm)
	PURITY = list_to_string(tssb.purity)
	
	FNAME_DATA = fin
	#print(" ".join(map(str,['/home/tzh232/subclonesim/method1/mh.o', MH_ITR, MH_STD, NDATA, NDELTA,NNODES, TREE_HEIGHT,FNAME_DATA, NTPS,NRATE])))
	#call(['./mh.o', '-d', './vw/'+fname, '-c', '--passes', '25', '--l1', '1e-6', '--l2', '0', '--readable_model', './weights/'+fname])
	call(['./mh.o', MH_ITR, MH_STD, NDATA, NDELTA,NNODES, TREE_HEIGHT,FNAME_DATA, NTPS,NRATE,NORM,PURITY])
