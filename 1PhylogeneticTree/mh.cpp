...
std::vector<double> splitStr(std::string str, char delimiter) {
    std::vector<double> internal;
    std::stringstream ss(str);
    std::string tok;
    
    while(getline(ss, tok, delimiter)) {
        internal.push_back(std::stof(tok));
    }
...
int main(int argc, char* argv[]){
	...
	printf("%d %.4f %d %d %d %d\n", conf.MH_ITR,conf.MH_STD,
		conf.NDATA,conf.NDELTA,conf.NNODES,conf.TREE_HEIGHT);
	conf.NTPS = atoi(argv[8]); // no. of samples
    conf.NRATE = splitStr(argv[9],',');
    for(int i = 0; i<conf.NRATE.size();i++){
        printf("%.3f ",conf.NRATE[i]);
    }
    cout << endl;
    
    conf.NORM = splitStr(argv[10],',');
    for(int i = 0; i<conf.NORM.size();i++){
        printf("%.3f ",conf.NORM[i]);
    }
    cout << endl;
    
    conf.PURITY = splitStr(argv[11],',');
    for(int i = 0; i<conf.PURITY.size();i++){
        printf("%.3f ",conf.PURITY[i]);
    }
    cout << endl;
    
	datum * data = new datum[conf.NDATA];
	load_data(FNAME_DATA,data,conf);
	
	node * nodes = new node[conf.NNODES];
...
double param_post(struct node nodes[], struct datum data[], int old,struct config conf, int tp){	
    std::vector<double> rate=conf.NRATE;
    std::vector<double> norm=conf.NORM;
    std::vector<double> purity=conf.PURITY;
    double normf=conf.NORM[tp];
    double purityf=conf.PURITY[tp];
    double llh = 0.0;
	for(int i=0;i<conf.NNODES;i++){
		double p=0;
		if(old==0)
		...
		for(int j=0;j<nodes[i].ndata;j++){
			llh+=data[nodes[i].dids.at(j)].log_ll(p,rate[nodes[i].dids.at(j)],tp,normf,purityf);
            //cout << "rate" << rate << endl;
		}
	}
	return llh;	
