...
def log_poisson_likelihood(a, x, phi, mu_r,norm,purity,logfaca):
    #print('phi=',phi,'mu_r=',mu_r)
    ### mu_r is the rate, whichi is m in paper notation
	mu = (a*(1-phi*purity)+ phi*purity * mu_r * a)*norm
	# factor = range(1,(x+1))
	# for x in factor:
	# 	logfaca+=log(x)

	likely = -mu+x*log(mu)-logfaca
	#print("likely",likely)
	return likely
...
def load_data(fname,prodfile):
	prod = numpy.load(prodfile).tolist()
	reader = csv.DictReader(open(fname), delimiter='\t')
	data = []
	id=0
	for row in reader:
		name = row['gene'] 
		a = [double(x) for x in row['a'].split(',')]       
		d = [int(x) for x in row['d'].split(',')]
		data.append(Datum(name, id, a, d,prod))
		id+=1
